...
class BloggerInterface:
    program_name = 'Reader2Blogger'
    program_version = '1.0'
    program = "%s-%s" % (program_name, program_version)

    def __init__(self):
        """Set up the blogger session and prepare for more commands"""
        self.blogger = service.GDataService()
        self.blogger.source = BloggerInterface.program
        self.blogger.service = 'blogger'
        self.blogger.server = 'www.blogger.com'

    def auth_url(self, next, scope='http://www.blogger.com/feeds',
                 secure=False, session-True):
        return self.blogger.GenerateAuthSubURL(next, scope, secure, session)
